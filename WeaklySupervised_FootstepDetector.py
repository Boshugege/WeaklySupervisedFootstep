...
class Config:
    ...
    def __init__(self):
        ...
        # ===== 模型参数 =====
        self.model_type = 'rf'          # 'rf' (Random Forest) 或 'gb' (Gradient Boosting)
...
class WeaklySupervisedDetector:
    ...
    def prepare_training_data(self, das_bands, audio_step_times, 
        ...
        pos_features, pos_valid = das_extractor.extract_features_at_times(
            das_bands, pos_times, window_s=0.2)
        neg_features, neg_valid = das_extractor.extract_features_at_times(
            das_bands, neg_times, window_s=0.2)
        
        # 合并
        X = np.vstack([pos_features, neg_features])
        y = np.concatenate([np.ones(len(pos_features)), 
